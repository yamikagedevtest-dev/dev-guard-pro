...
import { Trophy, Shield, AlertTriangle, ArrowLeft, Download, Award, CheckCircle } from "lucide-react";
...
<style>@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;600;700;800&display=swap');
*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Space Grotesk',sans-serif;background:#0b0d12;color:#e4e6ef;display:flex;align-items:center;justify-content:center;min-height:100vh}
.cert{width:900px;padding:64px;border:1px solid rgba(45,212,191,0.3);border-radius:24px;background:linear-gradient(145deg,#12151f 0%,#0b0d12 50%,#0f1220 100%);position:relative;overflow:hidden}
.cert::before{content:'';position:absolute;top:-40%;left:-40%;width:180%;height:180%;background:radial-gradient(circle at 30% 30%,rgba(45,212,191,0.04) 0%,transparent 60%)}
.header{text-align:center;margin-bottom:48px;position:relative}.logo{font-size:14px;color:rgba(45,212,191,0.7);font-weight:600;letter-spacing:3px;text-transform:uppercase;margin-bottom:16px}
.title{font-size:28px;font-weight:800;background:linear-gradient(135deg,#2dd4bf,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
...
.score-val{font-size:40px;font-weight:800;background:linear-gradient(135deg,#2dd4bf,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.score-label{font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:2px;margin-top:4px}
.skills{text-align:center;margin:24px 0}.skill-badge{display:inline-block;padding:5px 14px;border:1px solid rgba(45,212,191,0.2);border-radius:20px;margin:4px;font-size:12px;color:#2dd4bf}
...
.footer-left{font-size:12px;color:#6b7280;line-height:1.8}.cert-id{font-family:'JetBrains Mono',monospace;color:#2dd4bf}
.divider{width:80px;height:2px;background:linear-gradient(90deg,#2dd4bf,transparent);margin:0 auto 32px}
...
<script>window.print();</script></body></html>`;
  ...
  const statusColor = session?.cheat_status === 'clean' ? 'text-primary' : session?.cheat_status === 'suspicious' ? 'text-warning' : 'text-destructive';
    ...
    <div className="min-h-screen p-3 sm:p-4 md:p-8">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="text-center mb-8 sm:mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-2xl gradient-primary mb-4">
            <Trophy className="w-8 h-8 sm:w-10 sm:h-10 text-primary-foreground" />
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold mb-1">Test Results</h1>
        ...
        <div className="grid grid-cols-3 gap-2.5 sm:gap-4 mb-6 sm:mb-8">
          <Card className="glass text-center card-hover">
            <CardContent className="pt-4 sm:pt-6 pb-3 sm:pb-4 px-2 sm:px-6">
              <div className="text-2xl sm:text-4xl font-extrabold gradient-text">{Math.round(score)}%</div>
              <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">Score</p>
            </CardContent>
          </Card>
          <Card className="glass text-center card-hover">
            <CardContent className="pt-4 sm:pt-6 pb-3 sm:pb-4 px-2 sm:px-6">
              <div className="text-2xl sm:text-4xl font-extrabold text-primary">{Math.round(trust)}</div>
              <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">Trust Score</p>
            </CardContent>
          </Card>
          <Card className="glass text-center card-hover">
            <CardContent className="pt-4 sm:pt-6 pb-3 sm:pb-4 px-2 sm:px-6">
              <div className={`text-lg sm:text-2xl font-bold ${statusColor}`}>{session?.cheat_status?.toUpperCase()}</div>
              <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">Status</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 mb-6 sm:mb-8">
          <Card className="glass">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm sm:text-base flex items-center gap-2">
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg gradient-primary flex items-center justify-center"><Shield className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary-foreground" /></div>
                MCQ Round
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2.5 sm:space-y-3 text-xs sm:text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Correct</span><span className="font-medium text-primary">{mcqCorrect}</span></div>
          ...
          <Card className="glass">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm sm:text-base flex items-center gap-2">
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg gradient-accent flex items-center justify-center"><CodeIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-accent-foreground" /></div>
                Coding Round
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2.5 sm:space-y-3 text-xs sm:text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Submitted</span><span className="font-medium text-primary">{codingAnswers.filter(a => a.code_submission).length}</span></div>
            </CardContent>
          </Card>
        </div>
        ...
        {/* AI Verdict */}
        {session?.ai_verdict && (
          <Card className="glass mb-6 sm:mb-8">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm sm:text-base flex items-center gap-2">🤖 AI Analysis</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-xs sm:text-sm">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
                <div className="p-2.5 sm:p-3 rounded-lg bg-secondary/30 text-center">
                  <div className="text-base sm:text-lg font-bold">{(session.ai_verdict as any).humanLikelihood || 0}%</div>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">Human Likelihood</p>
                </div>
                <div className="p-2.5 sm:p-3 rounded-lg bg-secondary/30 text-center">
                  <div className="text-base sm:text-lg font-bold">{(session.ai_verdict as any).aiGeneratedProbability || 0}%</div>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">AI Generated</p>
                </div>
                <div className="p-2.5 sm:p-3 rounded-lg bg-secondary/30 text-center">
                  <div className="text-base sm:text-lg font-bold">{(session.ai_verdict as any).copiedProbability || 0}%</div>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">Copied</p>
                </div>
                <div className="p-2.5 sm:p-3 rounded-lg bg-secondary/30 text-center">
                  <div className={`text-base sm:text-lg font-bold ${(session.ai_verdict as any).finalVerdict === 'CLEAN' ? 'text-primary' : (session.ai_verdict as any).finalVerdict === 'SUSPICIOUS' ? 'text-warning' : 'text-destructive'}`}>
                    {(session.ai_verdict as any).finalVerdict}
                  </div>
                  <p className="text-[10px] sm:text-xs text-muted-foreground">Verdict</p>
                </div>
              </div>
        ...
        {/* Certificate */}
        <Card className="glass glow mb-6 sm:mb-8">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-sm sm:text-base"><Award className="w-4 h-4 sm:w-5 sm:h-5 text-primary" /> Certificate</CardTitle>
          </CardHeader>
          <CardContent>
            {certificate ? (
              <div className="flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
                <div className="space-y-2 text-xs sm:text-sm">
                  ...
                  <Button onClick={downloadCertificate} className="mt-3 gradient-primary text-primary-foreground text-sm">
                    <Download className="w-4 h-4 mr-2" /> Download Certificate
                  </Button>
                </div>
                <div className="bg-white p-3 rounded-xl">
                  <QRCodeSVG value={`${window.location.origin}/verify/${certificate.certificate_id}`} size={100} level="H" />
                </div>
              </div>
            ) : (
              <div className="text-center py-4">
                <p className="text-muted-foreground mb-4 text-xs sm:text-sm">Generate your verified certificate with QR code</p>
                <Button onClick={generateCertificate} disabled={generating} className="gradient-primary text-primary-foreground text-sm">
          ...
          <Card className="glass border-destructive/20">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-destructive text-sm sm:text-base">
                <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5" /> Violations ({violations.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {violations.map((v, i) => (
                  <div key={i} className="flex items-center justify-between text-xs sm:text-sm p-2 sm:p-2.5 rounded-lg bg-destructive/5">
                    <span>{v.violation_type.replace(/_/g, ' ')}</span>
                    <span className="text-muted-foreground text-[10px] sm:text-xs">{new Date(v.created_at).toLocaleTimeString()}</span>
                  </div>
                ))}
              </div>
            </CardContent>
